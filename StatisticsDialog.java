...
public class StatisticsDialog extends JDialog {
	...
	private JPanel createAnswerPanel() {
		...
		if (model.getStatistics().getCurrentStreak() == 0) {
			JLabel answer = new JLabel("You failed to guess the secret word "+model.getWord()+" in the allotted "+model.getMaximumRows()+" guesses");
			panel.add(answer);
		}
		return panel;
	}
	...
	private JPanel createButtonPanel() {
		JPanel panel = new JPanel(new FlowLayout());
		panel.setBorder(BorderFactory.createEmptyBorder(0, 5, 5, 5));//HERE IS WHERE THE BOARD SIZE IS 
